...
class Penjualan extends BaseController
{
    ...
    {
        $data=[
            'judulHalaman'=>'<i class="mdi mdi-cart-outline"></i> Product Sales',
        ...
        if($this->validate($validasiForm)){
            if(session()->get('IdPenjualan')==null) {
                ...
                {                
                    ...
                    return redirect()->to(site_url('/penjualan'))->with('pesan','<div class="alert alert-warning">Product saved successfully</div>');
                } else {
                    return redirect()->to(site_url('/penjualan'))->with('pesan','<div class="alert alert-warning">The number of items purchased exceeds stock</div>');
                }
            } else {
    ...
    public function formBayar(){        
        $data=[
            'judulHalaman'=>'<i class="mdi mdi-apple-keyboard-caps"></i> Payment',
